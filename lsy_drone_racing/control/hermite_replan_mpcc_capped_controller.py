...
class HermiteReplanMPCCCappedController(Controller):
    """
    ...
    Run example:
        python scripts/sim.py --config level2.toml --controller hermite_replan_mpcc_capped_controller.py --render=True
    """
    ...
    def _interpolate_arrays(path: np.ndarray, path_s: np.ndarray, s_query: float) -> tuple[np.ndarray, np.ndarray]:
        ...
        tangent = HermiteReplanMPCCCappedController._normalize(path[idx + 1] - path[idx])
