...
class TubeHermiteVisualController(Controller):
    """
    ...
    Run example:
        python scripts/sim.py --config level2.toml --controller tube_hermite_visual_controller.py --render=True
    """
    ...
    def __init__(self, obs: dict[str, Any], info: dict | None, config: Any):
        ...
        self.exit_dist = 0.72
        self.future_gate_lookahead = 2  # include active gate plus this many future gates

        # Hermite tube sampling. The tube centerline is a smooth function of the
        # currently known/filtered gate poses. Whenever a gate becomes known
        # exactly, the filtered gate frame changes and this Hermite is rebuilt.
        self.hermite_samples_per_segment = 20
        self.hermite_tangent_scale = 0.42
        self.hermite_max_tangent_fraction = 0.72
        self.tube_recalc_signature: tuple[Any, ...] | None = None
        ...
        self.debug_waypoints = np.empty((0, 3), dtype=np.float32)
        self.debug_tube_rings: list[np.ndarray] = []
        self.debug_tube_rails: list[np.ndarray] = []
    ...
    def _hermite_segment(p0: np.ndarray, p1: np.ndarray, m0: np.ndarray, m1: np.ndarray, n: int) -> np.ndarray:
        u = np.linspace(0.0, 1.0, max(2, int(n)), endpoint=False, dtype=np.float32).reshape(-1, 1)
        u2 = u * u
        u3 = u2 * u
    ...
    def _compute_hermite_tangents(self, points: np.ndarray) -> np.ndarray:
        points = np.asarray(points, dtype=np.float32).reshape((-1, 3))
        ...
        for i in range(n):
            ...
            tangent = self.hermite_tangent_scale * raw
            tangent = self._clip_norm(tangent, self.hermite_max_tangent_fraction * max(local_len, 1e-6))
            tangents[i] = tangent.astype(np.float32)
        return tangents.astype(np.float32)

    def _sample_hermite_centerline(self, waypoints: np.ndarray) -> np.ndarray:
        waypoints = self._remove_near_duplicates(waypoints, min_dist=0.035)
        if len(waypoints) <= 1:
            return waypoints.astype(np.float32)

        tangents = self._compute_hermite_tangents(waypoints)
        ...
        for i in range(len(waypoints) - 1):
            pieces.append(
                self._hermite_segment(
                    ...
                    tangents[i],
                    tangents[i + 1],
                    self.hermite_samples_per_segment,
                )
            )
        pieces.append(waypoints[-1].reshape(1, 3))
        return self._remove_near_duplicates(np.vstack(pieces).astype(np.float32), min_dist=0.008)

    def _build_tube_centerline(self, obs: dict[str, Any]) -> tuple[np.ndarray, list[dict[str, Any]]]:
        """
        Build the tube centerline as a Hermite curve through gate-derived waypoints.

        The only high-level geometry is the gate list:
            approach_i = gate_i - approach_dist * gate_x_i
            center_i   = gate_i
            exit_i     = gate_i + exit_dist * gate_x_i

        Since _filtered_gate_frame() is driven by obs["gates_pos"], obs["gates_quat"],
        and obs["gates_visited"], the Hermite automatically changes when a gate
        becomes known/exact. This is the intended "new data updates the tube" behavior.
        """
        ...
        for gate_i in range(target_gate, end_gate):
            ...
            # Every gate contributes an approach-center-exit mini-corridor. The
            # Hermite interpolation connects consecutive gate corridors smoothly.
            waypoints.extend([approach.astype(np.float32), center.astype(np.float32), exit_pt.astype(np.float32)])

            gate_infos.append(
                {
                    "i": int(gate_i),
                    "center": center.astype(np.float32),
                    "gate_x": gate_x.astype(np.float32),
                    "gate_y": gate_y.astype(np.float32),
                    "gate_z": gate_z.astype(np.float32),
                    "approach": approach.astype(np.float32),
                    "exit": exit_pt.astype(np.float32),
                }
            )

        wp = self._remove_near_duplicates(np.vstack(waypoints).astype(np.float32), 0.04)
        centerline = self._sample_hermite_centerline(wp)
        self.debug_waypoints = wp.astype(np.float32)
        return centerline.astype(np.float32), gate_infos
    ...
    def _tube_basis(self, tangent: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        tangent = self._normalize(tangent)
        world_z = np.array([0.0, 0.0, 1.0], dtype=np.float32)
        n1 = np.cross(tangent, world_z)
        if float(np.linalg.norm(n1)) < 1e-5:
            n1 = np.array([1.0, 0.0, 0.0], dtype=np.float32)
        n1 = self._normalize(n1)
        n2 = self._normalize(np.cross(tangent, n1), np.array([0.0, 0.0, 1.0], dtype=np.float32))
        return n1.astype(np.float32), n2.astype(np.float32)

    def _build_tube_visualization(
        self,
        ...
        """
        Approximate the tube surface with rings and longitudinal rails.

        MuJoCo debug drawing does not support translucent cylinders here, so we
        draw a wireframe tube. Rings show the local tube radius; rails show how
        the tube bends along the Hermite centerline.
        """
        if len(centerline) < 2 or len(path_s) < 2:
            return [], [], []

        rings: list[np.ndarray] = []
        rails_by_angle: list[list[np.ndarray]] = [[] for _ in range(4)]
        gate_lines: list[np.ndarray] = []

        # Ring spacing is distance-based so the visualization stays readable even
        # if the Hermite sampling density changes.
        total_s = float(path_s[-1])
        if total_s <= 1e-6:
            return [], [], []
        ring_spacing = 0.28
        ring_s_values = np.arange(0.0, total_s + 1e-6, ring_spacing, dtype=np.float32)
        if len(ring_s_values) == 0 or abs(float(ring_s_values[-1]) - total_s) > 1e-3:
            ring_s_values = np.concatenate([ring_s_values, np.array([total_s], dtype=np.float32)])

        angles = np.linspace(0.0, 2.0 * np.pi, 13, endpoint=True, dtype=np.float32)
        rail_angles = [0.0, 0.5 * np.pi, np.pi, 1.5 * np.pi]

        for s_val in ring_s_values:
            c, tangent = self._interpolate_path(centerline, path_s, float(s_val))
            r = self._nearest_gate_radius(c, gate_infos)
            n1, n2 = self._tube_basis(tangent)
            ring_pts = []
            for a in angles:
                ring_pts.append((c + r * (np.cos(a) * n1 + np.sin(a) * n2)).astype(np.float32))
            rings.append(np.vstack(ring_pts).astype(np.float32))

            for j, a in enumerate(rail_angles):
                rails_by_angle[j].append((c + r * (np.cos(a) * n1 + np.sin(a) * n2)).astype(np.float32))

        rails = [np.vstack(pts).astype(np.float32) for pts in rails_by_angle if len(pts) >= 2]

        # Gate normal markers: these make it obvious that the tube is a function
        # of the gate frames, not just straight Euclidean interpolation.
        for info in gate_infos:
            center = info["center"]
            gate_x = info["gate_x"]
            gate_y = info["gate_y"]
            gate_z = info["gate_z"]
            gate_lines.append(np.vstack([center - 0.35 * gate_x, center + 0.35 * gate_x]).astype(np.float32))
            gate_lines.append(np.vstack([center - 0.18 * gate_y, center + 0.18 * gate_y]).astype(np.float32))
            gate_lines.append(np.vstack([center - 0.18 * gate_z, center + 0.18 * gate_z]).astype(np.float32))

        return rings, rails, gate_lines
    ...
    def compute_control(self, obs: dict[str, Any], info: dict | None = None) -> np.ndarray:
        ...
        self.debug_tube_rings, self.debug_tube_rails, self.debug_gate_lines = self._build_tube_visualization(
            centerline, path_s, gate_infos
        )
    ...
    def render_callback(self, sim: Any) -> None:
        ...
        try:
            # Hermite tube centerline.
            if len(self.debug_centerline) >= 2:
                draw_line(sim, np.asarray(self.debug_centerline, dtype=np.float32), rgba=(0.0, 0.85, 1.0, 1.0))

            # Hermite waypoints: approach / center / exit for each visible gate.
            if len(self.debug_waypoints) >= 1:
                draw_points(sim, np.asarray(self.debug_waypoints, dtype=np.float32), rgba=(1.0, 0.75, 0.0, 1.0), size=0.026)

            # Tube wireframe: rings plus rails. Rings get tighter near gate centers.
            for ring in self.debug_tube_rings:
                if len(ring) >= 2:
                    draw_line(sim, np.asarray(ring, dtype=np.float32), rgba=(0.2, 0.7, 1.0, 0.55))
            for rail in self.debug_tube_rails:
                if len(rail) >= 2:
                    draw_line(sim, np.asarray(rail, dtype=np.float32), rgba=(0.2, 0.7, 1.0, 0.75))

            # Gate frame markers: local x/y/z at the gate center.
            for line in self.debug_gate_lines:
                if len(line) >= 2:
                    draw_line(sim, np.asarray(line, dtype=np.float32), rgba=(0.0, 1.0, 0.35, 1.0))
