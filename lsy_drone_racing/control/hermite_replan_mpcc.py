...
class HermiteReplanMPCCTightController(Controller):
    """
    ...
    Run example:
        python scripts/sim.py --config level2.toml --controller hermite_replan_mpcc_tight_controller.py --render=True
    """
    ...
    def __init__(self, obs: dict[str, Any], info: dict | None, config: Any):
        ...
        self.obstacle_clearance_seen = 0.48
        self.obstacle_clearance_nominal = 0.56  # bigger because nominal pose may still shift
        self.max_detours_per_segment = 2
        self.detour_min_segment_len = 0.20

        # A single detour waypoint lets the Hermite spline round the corner back
        # toward the obstacle. Use a short three-point corridor around each
        # obstacle: pre-side, side, post-side. This makes the sampled path itself
        # keep clearance, so the tracker is not fighting a bad curve.
        self.detour_corridor_half_len = 0.26
        self.detour_clearance_scale = 1.08
        ...
        # Important: store obstacle-side preference in WORLD XY coordinates,
        # not as +/- relative to the current segment. If the Hermite path is
        # rebuilt from a slightly different start pose, the local segment
        # direction can rotate or reverse; a segment-local sign can therefore
        # put the detour on the opposite real side of the obstacle.
        self.detour_side_memory: dict[int, float] = {}          # legacy/debug only
        self.detour_side_vector_memory: dict[int, np.ndarray] = {}
        self.detour_active_memory: dict[int, bool] = {}
        self.detour_side_lock_memory: dict[int, bool] = {}
        self.detour_unlock_clearance = 0.72
        self.detour_switch_margin = 0.22

        # ---------------------------
        # MPCC-style tracking parameters
        # ---------------------------
        self.cruise_speed = 0.58
        self.gate_speed = 0.38
        self.finish_speed = 0.35
        self.max_speed = 0.72
        self.min_speed = 0.25

        self.lookahead_min = 0.14
        self.lookahead_nominal = 0.30
        self.lookahead_max = 0.46
        ...
        # Critical anti-crossing guard: tube projection may only move forward by
        # this much arc length per control tick. Without this, if the Hermite path
        # crosses itself, the farthest-inside-tube rule can snap to the wrong
        # branch. These values are deliberately larger than cruise_speed * dt so
        # normal tracking is not choked, but small enough to prevent branch jumps.
        self.max_progress_advance_per_step = 0.095
        self.max_progress_advance_outside_tube = 0.045
        self.max_progress_advance_after_replan = 0.18
        ...
        self.target_slew_rate = 2.25       # m/s virtual target slew limit
        self.target_filter_max_lag = 0.22  # do not let the smoothed target fall too far behind
        ...
        self.velocity_ff_gain = 0.55
        self.use_accel_feedforward = False
        self.max_accel_ff = 1.2

        # Obstacle-aware tracking. Near obstacles the controller should behave
        # less like pure pursuit and more like MPCC: small lookahead, lower speed,
        # and an explicit cross-track correction toward the current path tube.
        self.obstacle_slow_radius = 1.05
        self.obstacle_tight_radius = 0.72
        self.obstacle_critical_radius = 0.56
        self.obstacle_tight_lookahead = 0.18
        self.obstacle_critical_lookahead = 0.13
        self.crosstrack_gain_nominal = 0.35
        self.crosstrack_gain_obstacle = 0.95
        self.crosstrack_max_nominal = 0.16
        self.crosstrack_max_obstacle = 0.32
    ...
    def _choose_committed_detour_side(
        self,
        obs_i: int,
        p0: np.ndarray,
        p1: np.ndarray,
        ...
        """
        Choose the obstacle side in a way that cannot flip between replans.

        The controller still adapts to the current segment by using either +perp
        or -perp, but the sign is chosen to match a persistent WORLD-frame side
        vector remembered for this obstacle. This fixes the old failure mode
        where the same stored sign meant a different physical side after the
        local segment direction changed.
        """
        obs_i = int(obs_i)
        perp = self._normalize(perp[:2], fallback=np.array([0.0, 1.0], dtype=np.float32))

        remembered = self.detour_side_vector_memory.get(obs_i)
        if remembered is not None:
            remembered = self._normalize(np.asarray(remembered, dtype=np.float32).reshape(2), fallback=perp)
            side_vec = perp if float(np.dot(perp, remembered)) >= 0.0 else -perp
            # Keep the original world-side commitment. Do not overwrite it with
            # the current segment's perpendicular, because that slowly rotating
            # memory can still walk across the obstacle on repeated replans.
            return side_vec.astype(np.float32)

        # First-time side selection. Prefer the side already closer to the current
        # command/vehicle, because crossing through the obstacle just to avoid it
        # is exactly what causes violent snaps. Fall back to the geometrically
        # natural side of the current segment if there is no useful anchor.
        anchor = self.filtered_target_pos if self.filtered_target_pos is not None else p0
        anchor_xy = np.asarray(anchor[:2], dtype=np.float32)
        o_xy = np.asarray(obstacle[:2], dtype=np.float32)

        plus_detour = o_xy + perp * self.obstacle_clearance_seen
        minus_detour = o_xy - perp * self.obstacle_clearance_seen
        plus_cost = float(np.linalg.norm(anchor_xy - plus_detour))
        minus_cost = float(np.linalg.norm(anchor_xy - minus_detour))

        if abs(plus_cost - minus_cost) > 0.04:
            side_vec = perp if plus_cost <= minus_cost else -perp
        else:
            away_xy = np.asarray(closest[:2] - o_xy, dtype=np.float32)
            away_n = float(np.linalg.norm(away_xy))
            if away_n > 1e-5:
                side_vec = perp if float(np.dot(away_xy / away_n, perp)) >= 0.0 else -perp
            else:
                side_vec = perp if (obs_i % 2 == 0) else -perp

        side_vec = self._normalize(side_vec, fallback=perp).astype(np.float32)
        self.detour_side_vector_memory[obs_i] = side_vec.copy()
        self.detour_side_memory[obs_i] = 1.0
        self.detour_side_lock_memory[obs_i] = True
        return side_vec
    ...
    def _insert_obstacle_detours(self, waypoints: np.ndarray, obs: dict[str, Any]) -> tuple[np.ndarray, np.ndarray]:
        ...
        for seg_i in range(len(waypoints) - 1):
            ...
            for obs_i, o in enumerate(obstacles):
                obs_i_int = int(obs_i)
                if obs_i_int in used_obstacles_this_replan:
                    # One committed detour per obstacle per plan. Multiple nearby
                    # segments adding the same obstacle is a major source of
                    # left/right oscillation around obstacle crossings.
                    continue
                ...
                side_vec = self._choose_committed_detour_side(obs_i_int, p0, p1, o, closest, perp)

                # If we already committed to one side and the drone is still near
                # this obstacle, do not switch sides. A side switch is allowed only
                # after we are comfortably clear; until then, the path should bend
                # continuously around the same side.
                drone_obs_dist = float(np.linalg.norm(drone_pos[:2] - o[:2]))
                if drone_obs_dist > self.detour_unlock_clearance and not was_active:
                    # Far away and inactive: permit a fresh choice next time this
                    # obstacle becomes relevant in a later track section.
                    self.detour_side_lock_memory[obs_i_int] = False

                # Use a slightly inflated clearance for the actual path. The
                # nominal/sensed obstacle center can still be imperfect, and the
                # inner state controller will always cut the corner a little.
                path_clearance = float(clearance * self.detour_clearance_scale)

                detour = closest.astype(np.float32).copy()
                detour[:2] = o[:2] + side_vec.astype(np.float32) * path_clearance
                ...
                # Build a short corridor around the side point. A single waypoint
                # causes a cubic spline to round/cut the obstacle corner. Three
                # waypoints keep the sampled Hermite path close to the safe side.
                dir_xy = self._normalize(d_xy, fallback=np.array([1.0, 0.0], dtype=np.float32))
                half_len = min(float(self.detour_corridor_half_len), 0.32 * seg_len_xy)
                entry = detour.copy()
                exit_pt = detour.copy()
                entry[:2] = detour[:2] - dir_xy * half_len
                exit_pt[:2] = detour[:2] + dir_xy * half_len

                corridor = np.vstack([entry, detour, exit_pt]).astype(np.float32)
                candidates.append((t, corridor, obs_i_int))

            if candidates:
                candidates.sort(key=lambda x: x[0])
                for _t, corridor, obs_i_int in candidates[: self.max_detours_per_segment]:
                    if obs_i_int in used_obstacles_this_replan:
                        continue
                    for detour_pt in corridor:
                        new_points.append(detour_pt.astype(np.float32))
                    detours_all.extend([pt.astype(np.float32) for pt in corridor])
                    seen_active_this_replan.add(obs_i_int)
                    used_obstacles_this_replan.add(obs_i_int)

        for obs_i in range(len(obstacles)):
            obs_i_int = int(obs_i)
            self.detour_active_memory[obs_i_int] = obs_i_int in seen_active_this_replan
    ...
    def _interpolate_arrays(path: np.ndarray, path_s: np.ndarray, s_query: float) -> tuple[np.ndarray, np.ndarray]:
        ...
        tangent = HermiteReplanMPCCTightController._normalize(path[idx + 1] - path[idx])
    ...
    def _rebuild_plan(self, obs: dict[str, Any], force: bool = False) -> None:
        ...
        if len(self.path) >= 2:
            ...
            # Replanning builds a new local path from the current position, but
            # crossings can still make a closest-point projection land far ahead.
            # Limit the first post-replan progress update too.
            replan_cap = float(old_progress_s + self.max_progress_advance_after_replan)
            self.progress_s = float(np.clip(s_proj, 0.0, min(float(self.path_s[-1]), replan_cap)))
    ...
    def _project_to_path_with_tube(self, pos: np.ndarray) -> tuple[float, np.ndarray, np.ndarray, float, bool]:
        """
        Progress projection with tube-based advancement and a forward cap.

        Tube advancement is useful because the drone should not have to hit every
        detour/control point exactly. But if the path crosses itself, the drone
        can be inside the tube of a much later branch. Therefore progress may
        only increase by a bounded arc length per control step. The controller
        can still move through the crossing, but it cannot teleport to the wrong
        future branch in one update.
        """
        if len(self.path) == 0:
        ...
        prev_progress = float(self.progress_s)
        min_allowed_s = max(0.0, prev_progress - float(self.progress_back_allowance))

        # The cap is the key fix for self-crossing paths. A branch that is far
        # ahead may be geometrically close, but it is not reachable in one tick.
        # Allow a slightly larger first update after replanning, because the path
        # coordinate system may have shifted.
        step_cap = (
            self.max_progress_advance_after_replan
            if self.just_replanned
            else self.max_progress_advance_per_step
        )
        max_allowed_s = min(float(self.path_s[-1]), prev_progress + float(step_cap))
        ...
        for i in range(len(self.path) - 1):
            ...
            if d2_tube <= tube_r2 and min_allowed_s <= s_here <= max_allowed_s:
        ...
        if farthest_inside is not None:
            ...
            self.progress_s = max(prev_progress, float(s_here))
            self.just_replanned = False
            return float(self.progress_s), p.astype(np.float32), tangent.astype(np.float32), float(d2), True

        # Outside the tube: do not let a nearby detour lobe or crossing branch pull
        # progress far forward/backward. Only a small forward catch-up is allowed.
        closest_s, closest_p, closest_tangent, closest_d2 = closest
        outside_max_s = min(float(self.path_s[-1]), prev_progress + float(self.max_progress_advance_outside_tube))
        if closest_s > self.progress_s:
            self.progress_s = float(min(closest_s, outside_max_s))
            p_clamped, tangent_clamped = self._interpolate_path(float(self.progress_s))
            self.just_replanned = False
            return float(self.progress_s), p_clamped.astype(np.float32), tangent_clamped.astype(np.float32), float(closest_d2), False
    ...
    def _nearest_obstacle_xy_distance(self, obs: dict[str, Any], pos: np.ndarray) -> float:
        obstacles, visited = self._obstacle_arrays(obs)
        if len(obstacles) == 0:
            return float("inf")
        ...
        for i, o in enumerate(obstacles):
            if not self.avoid_nominal_obstacles and not bool(visited[i]):
                continue
            best = min(best, float(np.linalg.norm(pos[:2] - o[:2])))
        return best
    ...
    def _tracking_speed_and_lookahead(self, obs: dict[str, Any], pos: np.ndarray, vel: np.ndarray, s_proj: float) -> tuple[float, float]:
        ...
        dist_obstacle = self._nearest_obstacle_xy_distance(obs, pos)
        ...
        if dist_gate < self.gate_tight_radius:
            ...
            lookahead = min(lookahead, 0.20)

        # Obstacle mode: do NOT use large pure-pursuit lookahead around a detour.
        # Large lookahead is exactly what makes the state controller cut the
        # inside of the curve and hit the obstacle.
        if dist_obstacle < self.obstacle_slow_radius:
            speed = min(speed, 0.44)
            lookahead = min(lookahead, 0.24)
        if dist_obstacle < self.obstacle_tight_radius:
            speed = min(speed, 0.34)
            lookahead = min(lookahead, self.obstacle_tight_lookahead)
        if dist_obstacle < self.obstacle_critical_radius:
            speed = min(speed, 0.27)
            lookahead = min(lookahead, self.obstacle_critical_lookahead)
        ...
        # If the drone is already moving fast, a tiny lookahead can destabilize
        # it, but do not allow velocity to enlarge lookahead near obstacles.
        vel_norm = float(np.linalg.norm(vel))
        if dist_obstacle >= self.obstacle_slow_radius:
            lookahead = max(lookahead, 0.18 + 0.16 * min(vel_norm, 1.5))
    ...
    def compute_control(self, obs: dict[str, Any], info: dict | None = None) -> np.ndarray:
        ...
        s_proj, projection, tangent_here, path_d2, inside_tube = self._project_to_path_with_tube(pos)
        speed, lookahead = self._tracking_speed_and_lookahead(obs, pos, vel, s_proj)
        dist_obstacle = self._nearest_obstacle_xy_distance(obs, pos)
        near_obstacle = dist_obstacle < self.obstacle_slow_radius

        # Target progress is usually monotonic, but near obstacles we must allow
        # the lookahead to shrink. Otherwise a previously large lookahead keeps
        # the setpoint far ahead and the drone cuts through the detour corner.
        if near_obstacle:
            s_target = min(max(float(self.progress_s), s_proj + lookahead), float(self.path_s[-1]))
        else:
        ...
        # Explicit contouring-error correction. Pure pursuit with a forward
        # target cuts corners; near obstacles that corner-cut can hit the pole.
        # Shift the target back toward the path projection, with a much stronger
        # correction in obstacle mode.
        path_error = projection.astype(np.float32) - pos.astype(np.float32)
        path_error[2] *= 0.55
        if near_obstacle:
            ct_gain = self.crosstrack_gain_obstacle
            ct_max = self.crosstrack_max_obstacle
        else:
            ct_gain = self.crosstrack_gain_nominal
            ct_max = self.crosstrack_max_nominal
        correction = self._clip_norm(ct_gain * path_error, ct_max)
        raw_target_pos = (raw_target_pos + correction).astype(np.float32)
        raw_target_pos[2] = max(float(raw_target_pos[2]), self.min_z)
        ...
        if self.filtered_target_pos is None or target_gate_changed or near_obstacle:
            # Near obstacles, command the obstacle-aware MPCC target directly.
            # Euclidean slew filtering can sweep the target through the obstacle
            # even when both old and new path samples are safe. Progress is
            # already capped, so the raw target should not teleport.
    ...
    def episode_reset(self) -> None:
        ...
        self.filtered_target_pos = None
        self.just_replanned = False
        self.detour_side_memory.clear()
        self.detour_side_vector_memory.clear()
        self.detour_active_memory.clear()
        self.detour_side_lock_memory.clear()
