...
class HermiteReplanMPCCController(Controller):
    """
    ...
    Run example:
        python scripts/sim.py --config level2.toml --controller hermite_replan_mpcc_controller.py --render=True
    """
    ...
    def __init__(self, obs: dict[str, Any], info: dict | None, config: Any):
        ...
        # Planner parameters
        # ---------------------------
        self.replan_every_steps = 5       # 10 Hz at env.freq=50; still reactive, less jittery
    ...
    def _insert_obstacle_detours(self, waypoints: np.ndarray, obs: dict[str, Any]) -> tuple[np.ndarray, np.ndarray]:
        ...
        for seg_i in range(len(waypoints) - 1):
            ...
            for obs_i, o in enumerate(obstacles):
                ...
                if dist_xy < 1e-5:
                    # Pick a deterministic perpendicular side if the segment runs directly through the obstacle.
                    perp = np.array([-d_xy[1], d_xy[0]], dtype=np.float32)
                    side = 1.0 if ((seg_i + obs_i) % 2 == 0) else -1.0
                    away_xy = side * self._normalize(perp)
                else:
                    away_xy = away_xy / dist_xy

                detour = closest.astype(np.float32).copy()
                detour[:2] = o[:2] + away_xy.astype(np.float32) * clearance
                ...
                if float(np.linalg.norm(detour - p0)) < 0.15 or float(np.linalg.norm(detour - p1)) < 0.15:
                    continue
                candidates.append((t, detour.astype(np.float32)))

            if candidates:
                candidates.sort(key=lambda x: x[0])
                for _t, detour in candidates[: self.max_detours_per_segment]:
    ...
    def _rebuild_plan(self, obs: dict[str, Any], force: bool = False) -> None:
        ...
        if not force and sig == self.last_obs_signature:
            return
        ...
        # The plan is rebuilt from the current drone pose, so arc-length progress
        # starts at zero for the new local plan. The tube projection below can
        # immediately jump this forward if the drone already lies inside a later
        # part of the replanned path corridor.
        self.progress_s = 0.0
        self.last_target_s = 0.0
    ...
    def compute_control(self, obs: dict[str, Any], info: dict | None = None) -> np.ndarray:
        ...
        # Force periodic replanning from the current drone pose. Also replan immediately
        # when gates/obstacles become exact or target_gate changes.
        sig = self._obs_signature(obs)
        force_replan = (self.tick % self.replan_every_steps == 0) or (sig != self.last_obs_signature)
        ...
        target_pos, tangent_target = self._interpolate_path(s_target)
        target_pos[2] = max(float(target_pos[2]), self.min_z)

        des_vel = speed * tangent_target
        des_vel = self._clip_norm(des_vel, self.max_speed)
        if not self.use_velocity_feedforward:
            des_vel[:] = 0.0
        else:
            des_vel = self.velocity_ff_gain * des_vel
