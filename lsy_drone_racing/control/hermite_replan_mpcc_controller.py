...
class HermiteReplanMPCCController(Controller):
    ...
    def __init__(self, obs: dict[str, Any], info: dict | None, config: Any):
        ...
        # Planner parameters
        # ---------------------------
        self.replan_every_steps = 2       # 25 Hz at env.freq=50; cheap and reactive
    ...
    def compute_control(self, obs: dict[str, Any], info: dict | None = None) -> np.ndarray:
        ...
        s_proj, projection, tangent_here, _d2 = self._project_to_path(pos)
        speed, lookahead = self._tracking_speed_and_lookahead(obs, pos, vel, s_proj)

        s_target = min(s_proj + lookahead, float(self.path_s[-1]))
        ...
        self.debug_tangent_line = np.vstack([projection, projection + 0.35 * tangent_here]).astype(np.float32)
