...
class HermiteReplanMPCCCommittedController(Controller):
    """
    ...
    Run example:
        python scripts/sim.py --config level2.toml --controller hermite_replan_mpcc_committed_controller.py --render=True
    """
    ...
    def _insert_obstacle_detours(self, waypoints: np.ndarray, obs: dict[str, Any]) -> tuple[np.ndarray, np.ndarray]:
        ...
        for seg_i in range(len(waypoints) - 1):
            ...
            for obs_i, o in enumerate(obstacles):
                ...
                detour[:2] = o[:2] + side_vec.astype(np.float32) * clearance
    ...
    def _interpolate_arrays(path: np.ndarray, path_s: np.ndarray, s_query: float) -> tuple[np.ndarray, np.ndarray]:
        ...
        tangent = HermiteReplanMPCCCommittedController._normalize(path[idx + 1] - path[idx])
